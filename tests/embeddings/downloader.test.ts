...
describe("ModelDownloader", () => {
  ...
  describe("getModelDir", () => {
    ...
    it("should return different paths for different model types", () => {
      ...
      const dir2 = downloader.getModelDir(ONNXModelType.NOMIC_V15);
  ...
  describe("isModelDownloaded", () => {
    ...
    it("should return true when model files exist", () => {
      ...
      fs.writeFileSync(path.join(modelDir, "vocab.txt"), "test");
    ...
    it("should return false when only vocab.txt exists", () => {
      const modelDir = downloader.getModelDir(ONNXModelType.DEFAULT);
      fs.mkdirSync(modelDir, { recursive: true });
      fs.writeFileSync(path.join(modelDir, "vocab.txt"), "test");
  ...
  describe("getModelInfo", () => {
    ...
    it("should return model info for NOMIC model", () => {
      const info = downloader.getModelInfo(ONNXModelType.NOMIC_V15);
      expect(info.name).toBe("nomic-embed-text-v1.5");
      expect(info.dimension).toBe(768);
    });

    it("should return model info for all new models", () => {
      const modelTypes = [ONNXModelType.NOMIC_V15];
    ...
    it("should have correct dimensions for models", () => {
      ...
      expect(downloader.getModelInfo(ONNXModelType.NOMIC_V15).dimension).toBe(768);
    });
  });
  ...
  describe("getModelPaths", () => {
    it("should return correct file paths", () => {
      ...
      expect(paths.vocabPath).toContain("vocab.txt");
    ...
    it("should return paths within model directory", () => {
      ...
      expect(!paths.vocabPath || paths.vocabPath.startsWith(modelDir)).toBe(true);
      expect(!paths.configPath || paths.configPath.startsWith(modelDir)).toBe(true);
    });
  });
  ...
  describe("getAllModels", () => {
    it("should return all available model types", () => {
      ...
      expect(models).toContain(ONNXModelType.NOMIC_V15);
    });
  });
  ...
  describe("getDownloadedModels", () => {
    ...
    it("should return downloaded models", () => {
      ...
      fs.writeFileSync(path.join(modelDir1, "vocab.txt"), "test");

      const modelDir2 = downloader.getModelDir(ONNXModelType.NOMIC_V15);
      fs.mkdirSync(modelDir2, { recursive: true });
      fs.writeFileSync(path.join(modelDir2, "model.onnx"), "test");
      fs.writeFileSync(path.join(modelDir2, "vocab.txt"), "test");

      const downloaded = downloader.getDownloadedModels();
      expect(downloaded.length).toBe(2);
      expect(downloaded).toContain(ONNXModelType.DEFAULT);
      expect(downloaded).toContain(ONNXModelType.NOMIC_V15);
    });
  });
  ...
  describe("File size estimation", () => {
    it("should return correct file sizes for all models", () => {
      const testCases = [
        ...
        { model: ONNXModelType.NOMIC_V15, expectedSize: 547 },
      ];
